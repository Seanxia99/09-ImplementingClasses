...
class Point(object):
    ...
    def move_to(self, x, y):
        self.mtd = self.mtd + math.sqrt((self.x - x) ** 2 + (self.y - y) ** 2)
        self.x = x
        self.y = y
        self.mtx = x
        self.mty = y
    ...
    def move_by(self, x, y):
        self.mbd = self.mbd + math.sqrt((self.x - (self.x + x)) ** 2 + (self.y - (self.y + y)) ** 2)
        self.x = x
        self.y = y
        self.mbx = self.x + x
        self.mby = self.y + y
    ...
    def get_distance_from(self, p1):
        distance = math.sqrt((p1.x - self.x) ** 2 + (p1.y - self.y) ** 2)
        return distance

    def get_distance_from_start(self):
        distance = math.sqrt((self.x - self.x0) ** 2 + (self.y - self.y0) ** 2)
        return distance

    def get_distance_traveled(self):
        distance = self.mtd + self.mbd
        return distance
